...
import Illustration21Src1024 from '@snowui-design-system/resource-base/assets/illustrations/illustration-21-1024.png';
  ...
  let { width = 320, height, alt = "Illustration21", class: className = "", ...rest }: BackgroundProps = $props();
  let resolvedWidth = $derived(width ?? 320);
  // 根据 width 选择对应宽度的图片，如果没有精确匹配则使用最接近的宽度
  let closestWidth = $derived(findClosestWidth(resolvedWidth, Illustration21AvailableWidths));
  let imageSrc = $derived(Illustration21WidthMap[closestWidth as keyof typeof Illustration21WidthMap] ?? Illustration21Src);
</script>
<img
  src={imageSrc}
  alt={alt}
