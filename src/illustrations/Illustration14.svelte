...
import Illustration14Src1024 from '@snowui-design-system/resource-base/assets/illustrations/illustration-14-1024.png';
  ...
  let { width = 320, height, alt = "Illustration14", class: className = "", ...rest }: BackgroundProps = $props();
  let resolvedWidth = $derived(width ?? 320);
  // 根据 width 选择对应宽度的图片，如果没有精确匹配则使用最接近的宽度
  let closestWidth = $derived(findClosestWidth(resolvedWidth, Illustration14AvailableWidths));
  let imageSrc = $derived(Illustration14WidthMap[closestWidth as keyof typeof Illustration14WidthMap] ?? Illustration14Src);
</script>
<img
  src={imageSrc}
  alt={alt}
