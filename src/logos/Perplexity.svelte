<script lang="ts">
  import type { AvatarProps } from '../lib/types';

  let { size = 32, width, height, alt = "Perplexity", class: className = "", ...rest }: AvatarProps = $props();
  let resolvedWidth = $derived(size ?? width ?? 32);
  let resolvedHeight = $derived(size ?? height ?? 32);
</script>

<svg viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg"
  width={resolvedWidth}
  height={resolvedHeight}
  class={className}
  role={alt ? "img" : undefined}
  aria-label={alt || undefined}
  aria-hidden={alt ? undefined : "true"}
  {...rest}
>
<g clip-path="url(#clip0_7159_5282)">
<path d="M0 8C0 3.58172 3.58172 0 8 0H24C28.4183 0 32 3.58172 32 8V24C32 28.4183 28.4183 32 24 32H8C3.58172 32 0 28.4183 0 24V8Z" fill="currentColor"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M9.53154 5.75L15.3829 11.1447V5.76245H16.5219V11.1689L22.3994 5.75V11.9008H24.8125V20.7727H22.4068V26.2497L16.5219 21.0759V26.3091H15.3829V21.1612L9.53817 26.3125V20.7727H7.125V11.9008H9.53154V5.75ZM14.5242 13.0267H8.26402V19.6469H9.53674V17.5586L14.5242 13.0267ZM10.6771 18.0582V23.8002L15.3829 19.6528V13.7812L10.6771 18.0582ZM16.5546 19.5979V13.7756L21.2617 18.0529V20.7727H21.2678V23.7416L16.5546 19.5979ZM22.4068 19.6469H23.6735V13.0267H17.46L22.4068 17.5117V19.6469ZM21.2604 11.9008V8.33972L17.3979 11.9008H21.2604ZM14.533 11.9008H10.6706V8.33972L14.533 11.9008Z" fill="white"/>
</g>
<defs>
<clipPath id="clip0_7159_5282">
<rect width="32" height="32" fill="white"/>
</clipPath>
</defs>
</svg>
