...
import Gradient04Src1920 from '@snowui-design-system/resource-base/assets/backgrounds/gradient-04-1920.jpg';
  ...
  let { width = 1024, height, alt = "Gradient04", class: className = "", ...rest }: BackgroundProps = $props();
  let resolvedWidth = $derived(width ?? 1024);
  // 根据 width 选择对应宽度的图片，如果没有精确匹配则使用最接近的宽度
  let closestWidth = $derived(findClosestWidth(resolvedWidth, Gradient04AvailableWidths));
  let imageSrc = $derived(Gradient04WidthMap[closestWidth as keyof typeof Gradient04WidthMap] ?? Gradient04Src);
</script>
<img
  src={imageSrc}
  alt={alt}
